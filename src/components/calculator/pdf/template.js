...
const PdfCalculatorTemplate = ({ data, total }) => {
  ...
  const items = data.map((e) => {
    const selectItems = e.select.length? e.select.map(e => (
      <Text key={e} style={styles.featureSelectText}>{e}</Text>
    )) : (
      <Text key={e} style={styles.featureSelectText}>Not Feature Select</Text>
    );
    return(
      <View key={e.feature} style={styles.tableFeature}>
        ...
        <View style={styles.featureMidCol}>
          {selectItems}
        </View>
        <View style={styles.featureRightCol}>
          <Text>${' '}{e.min}{' '}-{' '}{e.max}</Text>
        </View>
      </View>
    );
  });
      ...
      <Page size="A4" style={styles.body}>
        ...
        <View style={styles.table}>
          ...
          <View style={styles.description}>
            <Text style={styles.descriptionText}>We partner with new and early-stage entrepreneurs and industry experts to transforms ideas into Web and Mobile Software Products. </Text>
            <Text style={styles.descriptionText}>Since day one, our technique is focused on early results and transparent communication</Text>
          </View>
