...
const StyledDiv = styled.div`
  height: 100px;
  background: linear-gradient(901deg, rgba(255, 255, 255, 0.1) 1%, rgba(255, 250, 251, 1) 90%);
  position: relative;
  top: -100px;
`;
  ...
  return (
    <>
      <Column isSize={{ mobile: 12, desktop: 4 }}>
        <CardStyled>
          <>
            <Img fluid={data.file.childImageSharp.gatsbyImageData} alt="" />
            <StyledDiv />
          </>
          <>
...
const query = graphql`
  query {
    file(relativePath: { eq: "service/details/keyboard.png" }) {
      childImageSharp {
